...
docker run --rm \
    -v `readlink -f $GOLOS_STATE/genesis.dat`:/golos.dat \
    -v `readlink -f $GOLOS_STATE/genesis.dat.map`:/golos.dat.map \
    -v `readlink -f $GOLOS_STATE/operation_dump`:/operation_dump \
    -v `readlink -f $DEST`:/genesis-data \
    $GOLOS_IMAGE bash -c \
    'ls -l / /opt/cyberway/bin/ /opt/golos.contracts /opt/golos.contracts/genesis /operation_dump / &&
     ...
     /opt/cyberway/bin/create-genesis -g genesis-info.json -d operation_dump -o /genesis-data/genesis.dat -e /genesis-data/event-genesis/ && \
